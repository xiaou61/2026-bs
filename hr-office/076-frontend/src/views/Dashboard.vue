<template>
  <div>
    <el-row :gutter="12" v-if="isAdmin">
      ...
      <el-col :span="6"><el-card><div class="card"><span>信息总数</span><b>{{ stats.serviceCount || 0 }}</b></div></el-card></el-col>
      <el-col :span="6"><el-card><div class="card"><span>处理总数</span><b>{{ stats.orderCount || 0 }}</b></div></el-card></el-col>
      <el-col :span="6"><el-card><div class="card"><span>支付总额</span><b>¥{{ Number(stats.totalAmount || 0).toFixed(2) }}</b></div></el-card></el-col>
    </el-row>
    <el-row :gutter="12" v-else>
      <el-col :span="6"><el-card><div class="card"><span>我的信息</span><b>{{ userStats.serviceCount || 0 }}</b></div></el-card></el-col>
      <el-col :span="6"><el-card><div class="card"><span>我的处理</span><b>{{ userStats.orderCount || 0 }}</b></div></el-card></el-col>
    ...
    <el-card style="margin-top: 12px" v-if="isAdmin">
      <template #header>热门处理信息</template>
      <el-table :data="hotServices" size="small">
        <el-table-column prop="title" label="企业信息" min-width="220" />
        <el-table-column prop="bookedCount" label="处理量" width="120" />
