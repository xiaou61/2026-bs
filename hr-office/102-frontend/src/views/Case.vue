<template>
  <DataPage ref="pageRef" title="案件台账" description="维护案件编号、委托人、承办律师、类型、阶段和下一步动作。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 0, priority: '普通' }" @row-action="handleAction" />
</template>
...
import { addLegalCase, advanceCase, closeCase, deleteLegalCase, getLegalCasePage, updateLegalCase } from '../api'
...
const api = { page: getLegalCasePage, add: addLegalCase, update: updateLegalCase, delete: deleteLegalCase }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'caseType', label: '案件类型' }, { type: 'select', prop: 'status', label: '状态', options: caseStatusOptions }, { type: 'select', prop: 'priority', label: '优先级', options: priorityOptions }]
const columns = [{ prop: 'caseNo', label: '案件编号', minWidth: 150 }, { prop: 'title', label: '标题', minWidth: 220 }, { prop: 'caseType', label: '类型' }, { prop: 'priority', label: '优先级', map: priorityMap }, { prop: 'status', label: '状态', map: caseStatusMap }, { prop: 'currentStage', label: '当前阶段' }, { prop: 'nextAction', label: '下一步', minWidth: 220, long: true }]
const formFields = [{ prop: 'clientId', label: '委托人ID', type: 'number', min: 1, required: true }, { prop: 'lawyerId', label: '律师ID', type: 'number', min: 1 }, { prop: 'title', label: '案件标题', required: true }, { prop: 'caseType', label: '案件类型' }, { prop: 'priority', label: '优先级', type: 'select', options: priorityOptions }, { prop: 'status', label: '状态', type: 'select', options: caseStatusOptions }, { prop: 'summary', label: '案情摘要', type: 'textarea', rows: 4 }, { prop: 'currentStage', label: '当前阶段' }, { prop: 'nextAction', label: '下一步动作', type: 'textarea', rows: 3 }]
const rowActions = [{ name: 'advance', label: '推进' }, { name: 'close', label: '结案', type: 'danger' }]

const handleAction = async (name, row) => {
  if (name === 'advance') await advanceCase(row.id)
  if (name === 'close') await closeCase(row.id)
