<template>
  <DataPage ref="pageRef" title="咨询预约" description="安排案件咨询预约，支持确认、取消和完成。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 0 }" @row-action="handleAction" />
</template>
...
import { addAppointmentRecord, cancelAppointment, confirmAppointment, deleteAppointmentRecord, finishAppointment, getAppointmentRecordPage, updateAppointmentRecord } from '../api'
...
const api = { page: getAppointmentRecordPage, add: addAppointmentRecord, update: updateAppointmentRecord, delete: deleteAppointmentRecord }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'caseId', label: '案件ID' }, { type: 'select', prop: 'status', label: '状态', options: appointmentOptions }]
const columns = [{ prop: 'caseId', label: '案件ID' }, { prop: 'appointmentTime', label: '预约时间', minWidth: 170 }, { prop: 'appointmentType', label: '方式' }, { prop: 'location', label: '地点', minWidth: 220 }, { prop: 'status', label: '状态', map: appointmentMap }, { prop: 'remark', label: '备注', minWidth: 180, long: true }]
const formFields = [{ prop: 'caseId', label: '案件ID', type: 'number', min: 1, required: true }, { prop: 'clientId', label: '委托人ID', type: 'number', min: 1 }, { prop: 'lawyerId', label: '律师ID', type: 'number', min: 1 }, { prop: 'appointmentTime', label: '预约时间', type: 'datetime' }, { prop: 'appointmentType', label: '预约方式' }, { prop: 'location', label: '地点' }, { prop: 'status', label: '状态', type: 'select', options: appointmentOptions }, { prop: 'remark', label: '备注', type: 'textarea', rows: 3 }]
...
const handleAction = async (name, row) => {
  if (name === 'confirm') await confirmAppointment(row.id)
  if (name === 'finish') await finishAppointment(row.id)
  if (name === 'cancel') await cancelAppointment(row.id)
