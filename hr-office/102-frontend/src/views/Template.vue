<template>
  <DataPage ref="pageRef" title="文书模板" description="维护法律文书模板、变量说明和启用状态。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 1, usageCount: 0 }" @row-action="handleAction" />
</template>
...
import { addDocumentTemplate, deleteDocumentTemplate, disableTemplate, enableTemplate, getDocumentTemplatePage, updateDocumentTemplate } from '../api'
...
const api = { page: getDocumentTemplatePage, add: addDocumentTemplate, update: updateDocumentTemplate, delete: deleteDocumentTemplate }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'templateType', label: '模板类型' }, { type: 'select', prop: 'status', label: '状态', options: statusOptions }]
const columns = [{ prop: 'templateName', label: '模板名称', minWidth: 200 }, { prop: 'templateType', label: '类型' }, { prop: 'usageCount', label: '使用次数' }, { prop: 'status', label: '状态', map: statusMap }, { prop: 'contentVariables', label: '变量说明', minWidth: 260, long: true }]
const formFields = [{ prop: 'templateName', label: '模板名称', required: true }, { prop: 'templateType', label: '模板类型' }, { prop: 'contentVariables', label: '变量说明', type: 'textarea', rows: 5 }, { prop: 'status', label: '状态', type: 'select', options: statusOptions }, { prop: 'usageCount', label: '使用次数', type: 'number', min: 0 }]
const rowActions = [{ name: 'enable', label: '启用', type: 'success' }, { name: 'disable', label: '停用', type: 'danger' }]

const handleAction = async (name, row) => {
  if (name === 'enable') await enableTemplate(row.id)
  if (name === 'disable') await disableTemplate(row.id)
