<template>
  <DataPage ref="pageRef" title="法律文书" description="生成、复核和归档案件法律文书。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 0 }" @row-action="handleAction" />
</template>
...
import { addLegalDocument, deleteLegalDocument, generateDocument, getLegalDocumentPage, reviewDocument, updateLegalDocument } from '../api'
...
const api = { page: getLegalDocumentPage, add: addLegalDocument, update: updateLegalDocument, delete: deleteLegalDocument }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'caseId', label: '案件ID' }, { type: 'select', prop: 'status', label: '状态', options: docOptions }]
const columns = [{ prop: 'documentNo', label: '文书编号', minWidth: 160 }, { prop: 'documentTitle', label: '标题', minWidth: 220 }, { prop: 'documentType', label: '类型' }, { prop: 'status', label: '状态', map: docMap }, { prop: 'reviewComment', label: '复核意见', minWidth: 220, long: true }]
const formFields = [{ prop: 'caseId', label: '案件ID', type: 'number', min: 1, required: true }, { prop: 'templateId', label: '模板ID', type: 'number', min: 1 }, { prop: 'documentTitle', label: '文书标题', required: true }, { prop: 'documentType', label: '文书类型' }, { prop: 'content', label: '正文内容', type: 'textarea', rows: 6 }, { prop: 'status', label: '状态', type: 'select', options: docOptions }, { prop: 'reviewComment', label: '复核意见', type: 'textarea', rows: 3 }]
const rowActions = [{ name: 'generate', label: '生成' }, { name: 'pass', label: '通过', type: 'success' }, { name: 'reject', label: '驳回', type: 'danger' }]

const handleAction = async (name, row) => {
  if (name === 'generate') await generateDocument(row.id)
  if (name === 'pass' || name === 'reject') {
    const { value } = await ElMessageBox.prompt('请输入复核意见', '文书复核', { inputValue: name === 'pass' ? '复核通过' : '复核驳回', confirmButtonText: '确定', cancelButtonText: '取消' })
    await reviewDocument({ id: row.id, status: name === 'pass' ? 2 : 3, reviewComment: value })
  }
