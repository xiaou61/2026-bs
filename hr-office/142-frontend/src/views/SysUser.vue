<template>
  <DataPage title="账号权限" description="维护系统管理员、理赔专员、车主客户和审核主管账号，以及部门、联系方式和启停状态" :api="api" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="defaults" :can-create="canManage" :can-edit="canManage" :can-delete="canManage" @row-action="handleAction" />
</template>
...
const formFields = [
  ...
  { prop: 'nickname', label: '姓名' },
  { prop: 'role', label: '角色', type: 'select', options: [{ label: '系统管理员', value: 'ADMIN' }, { label: '理赔专员', value: 'LEGAL' }, { label: '车主客户', value: 'APPLICANT' }, { label: '审核主管', value: 'APPROVER' }] },
  { prop: 'department', label: '部门' },
  { prop: 'phone', label: '电话' },
  { prop: 'email', label: '邮箱' },
...
const defaults = { status: 1, role: 'APPLICANT' }
