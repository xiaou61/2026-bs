...
class EnterpriseInfoManagementApplicationSmokeTest {
    ...
    void smokeTestLoginAuthorizationProcessReviewComplaintAndLogout() {
        ...
        ResponseEntity<Map> orderResponse = restTemplate.exchange(
                ...
                authEntity(buyer.token(), Map.of(
                        "serviceId", 3,
                        "quantity", 1,
                        "remark", "test enterprise info process",
                        "appointmentDate", LocalDate.now().plusDays(2).toString(),
                        "appointmentTime", "10:30",
                        "plateNo", "13900007600",
                        "vehicleModel", "王经理",
                        "faultDesc", "税务申报信息需要核验"
                )),
                Map.class
        );
        ...
        assertOk(restTemplate.exchange(
                "/api/review",
                HttpMethod.POST,
                authEntity(buyer.token(), Map.of("orderId", orderId.longValue(), "rating", 5, "content", "企业信息核验处理清晰高效")),
                Map.class
        ));
        ...
        assertOk(restTemplate.exchange(
                "/api/complaint",
                HttpMethod.POST,
                authEntity(buyer.token(), Map.of("orderId", orderId.longValue(), "type", "ORDER", "content", "需要补充处理凭证")),
                Map.class
        ));
