...
import com.autorepair.entity.Complaint;
...
import com.autorepair.mapper.ComplaintMapper;
...
import com.autorepair.vo.ComplaintVO;
...
public class ComplaintService {
    ...
    public void add(Complaint complaint, Long userId) {
        if (complaint == null || complaint.getOrderId() == null) {
            throw new BusinessException("请选择关联预约单");
        }
        ...
        RepairAppointment order = orderMapper.selectById(complaint.getOrderId());
        if (order == null) {
            throw new BusinessException("预约单不存在");
        }
        if (!order.getUserId().equals(userId)) {
            throw new BusinessException(403, "只能申诉自己的预约单");
        }
        if (order.getStatus() == null || (order.getStatus() != 1 && order.getStatus() != 2 && order.getStatus() != 3)) {
            throw new BusinessException("当前预约单状态不可申诉");
        }
        ...
        if (processingCount != null && processingCount > 0) {
            throw new BusinessException("该预约单已有待处理申诉");
        }
    ...
    private Page<ComplaintVO> convertComplaintPage(Page<Complaint> page) {
        ...
        Map<Long, RepairAppointment> orderMap = new HashMap<>();
        Set<Long> serviceIds = new HashSet<>();
        if (!orderIds.isEmpty()) {
            List<RepairAppointment> orders = orderMapper.selectBatchIds(orderIds);
    ...
    private void appendOrderNoFilter(QueryWrapper<Complaint> wrapper, String orderNo) {
        ...
        List<Long> orderIds = orders.stream().map(RepairAppointment::getId).collect(Collectors.toList());
