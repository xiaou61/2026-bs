...
(1, 'U202605030001', 'admin', '123456', '系统管理员', '13800000001', 'admin@petcafe.com', 2000.00, 0.00, 'ADMIN', 1, CURRENT_TIMESTAMP),
(2, 'U202605030002', 'staff', '123456', '店长小岚', '13800000002', 'staff@petcafe.com', 500.00, 0.00, 'STAFF', 1, CURRENT_TIMESTAMP),
(3, 'U202605030003', 'customer', '123456', '顾客小溪', '13800000003', 'customer@petcafe.com', 368.50, 131.50, 'CUSTOMER', 1, CURRENT_TIMESTAMP);
...
(1, 'RSV202605030001', 3, 1, 2, '顾客小溪', '13800000003', CURRENT_DATE, '14:00-15:30', 2, '无', '想靠近猫咪互动区', 'CONFIRMED', '已为您保留座位', DATEADD('HOUR', -4, CURRENT_TIMESTAMP), DATEADD('HOUR', -4, CURRENT_TIMESTAMP)),
(2, 'RSV202605030002', 3, 1, 1, '顾客小溪', '13800000003', DATEADD('DAY', -1, CURRENT_DATE), '16:00-17:00', 2, '无', '庆生小聚', 'COMPLETED', '已完成接待', DATEADD('DAY', -1, CURRENT_TIMESTAMP), DATEADD('DAY', -1, CURRENT_TIMESTAMP));
...
(1, 'CO202605030001', 3, 1, 1, 2, 'IN_STORE', 56.00, 56.00, 'COMPLETED', '少糖', DATEADD('HOUR', -3, CURRENT_TIMESTAMP), DATEADD('HOUR', -3, CURRENT_TIMESTAMP), DATEADD('HOUR', -3, CURRENT_TIMESTAMP)),
(2, 'CO202605030002', 3, 1, 2, 1, 'IN_STORE', 32.00, 32.00, 'WAIT_PAY', '生日插牌', NULL, DATEADD('MINUTE', -20, CURRENT_TIMESTAMP), DATEADD('MINUTE', -20, CURRENT_TIMESTAMP));

INSERT INTO payment_record (id, user_id, order_id, pay_no, pay_type, pay_amount, status, pay_time, create_time, update_time) VALUES
(1, 3, NULL, 'PAY202605030001', 'RECHARGE', 200.00, 'SUCCESS', DATEADD('DAY', -2, CURRENT_TIMESTAMP), DATEADD('DAY', -2, CURRENT_TIMESTAMP), DATEADD('DAY', -2, CURRENT_TIMESTAMP)),
(2, 3, 1, 'PAY202605030002', 'BALANCE', 56.00, 'SUCCESS', DATEADD('HOUR', -3, CURRENT_TIMESTAMP), DATEADD('HOUR', -3, CURRENT_TIMESTAMP), DATEADD('HOUR', -3, CURRENT_TIMESTAMP));
...
(1, 1, 1, 3, 5, '店宠很亲人，咖啡也很稳定。', 2, '感谢喜欢奶盖和我们的咖啡，欢迎下次再来。', 'REPLIED', DATEADD('MINUTE', -150, CURRENT_TIMESTAMP), DATEADD('MINUTE', -100, CURRENT_TIMESTAMP), DATEADD('MINUTE', -150, CURRENT_TIMESTAMP), DATEADD('MINUTE', -100, CURRENT_TIMESTAMP));

INSERT INTO system_notice (id, title, content, publisher_id, notice_type, status, create_time, update_time) VALUES
(1, '五一主题活动上线', '本周末到店可领取宠物咖啡拉花限定贴纸。', 1, 'SYSTEM', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(2, '花园店布景调整', '爪印花园咖啡馆本周进行露台布景更新。', 1, 'SYSTEM', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
