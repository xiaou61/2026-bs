<template>
  <el-card>
    ...
    <el-table :data="tableData" v-loading="loading">
      ...
      <el-table-column prop="orderNo" label="预约单号" min-width="180" />
      <el-table-column prop="serviceTitle" label="维修项目" min-width="160" />
  ...
  <el-dialog v-model="dialogVisible" title="新增申诉" width="520px">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="关联预约" prop="orderId">
        <el-select v-model="form.orderId" style="width: 100%" filterable placeholder="请选择预约单">
          <el-option v-for="order in orderOptions" :key="order.id" :label="`${order.orderNo} / ${order.serviceTitle || '维修项目'}`" :value="order.id" />
        </el-select>
      </el-form-item>
...
const rules = {
  orderId: [{ required: true, message: '请选择关联预约单', trigger: 'change' }],
  type: [
