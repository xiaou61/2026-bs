<template>
  <el-card>
    ...
    <el-table :data="tableData" v-loading="loading">
      ...
      <el-table-column label="角色" width="100">
        <template #default="{ row }"><el-tag>{{ row.role === 'ADMIN' ? '管理员' : '用户' }}</el-tag></template>
      </el-table-column>
  ...
  <el-dialog v-model="dialogVisible" :title="form.id ? '编辑用户' : '新增用户'" width="560px">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
      ...
      <el-form-item label="头像地址"><el-input v-model="form.avatar" maxlength="255" /></el-form-item>
      <el-form-item label="角色"><el-select v-model="form.role" :disabled="form.id === 1"><el-option label="管理员" value="ADMIN" /><el-option label="用户" value="USER" /></el-select></el-form-item>
...
const openAdd = () => {
  Object.assign(form, { id: null, username: '', password: '', nickname: '', email: '', avatar: '', role: 'USER', status: 1 })
