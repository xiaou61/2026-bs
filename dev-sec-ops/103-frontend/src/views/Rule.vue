<template>
  <DataPage ref="pageRef" title="质检规则" description="维护质检关键词、类型和扣分配置。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 1, deductScore: 5 }" @row-action="handleAction" />
</template>
...
import { addQualityRule, deleteQualityRule, getQualityRulePage, updateQualityRule } from '../api'
...
const api = { page: getQualityRulePage, add: addQualityRule, update: updateQualityRule, delete: deleteQualityRule }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'ruleType', label: '规则类型' }, { type: 'select', prop: 'status', label: '状态', options: statusOptions }]
const columns = [{ prop: 'ruleName', label: '规则名称', minWidth: 180 }, { prop: 'ruleType', label: '类型' }, { prop: 'keyword', label: '关键词' }, { prop: 'deductScore', label: '扣分' }, { prop: 'status', label: '状态', map: statusMap }]
const formFields = [{ prop: 'ruleName', label: '规则名称', required: true }, { prop: 'ruleType', label: '规则类型' }, { prop: 'keyword', label: '关键词' }, { prop: 'deductScore', label: '扣分', type: 'number', min: 0, precision: 2 }, { prop: 'status', label: '状态', type: 'select', options: statusOptions }, { prop: 'description', label: '说明', type: 'textarea', rows: 3 }]
const rowActions = []
