<template>
  <DataPage ref="pageRef" title="服务渠道" description="维护客服渠道和归属团队。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 1 }" @row-action="handleAction" />
</template>
...
import { addServiceChannel, deleteServiceChannel, getServiceChannelPage, updateServiceChannel } from '../api'
...
const api = { page: getServiceChannelPage, add: addServiceChannel, update: updateServiceChannel, delete: deleteServiceChannel }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'input', prop: 'channelType', label: '渠道类型' }, { type: 'select', prop: 'status', label: '状态', options: statusOptions }]
const columns = [{ prop: 'channelName', label: '渠道名称' }, { prop: 'channelType', label: '类型' }, { prop: 'ownerTeam', label: '归属团队' }, { prop: 'status', label: '状态', map: statusMap }, { prop: 'remark', label: '备注', minWidth: 220, long: true }]
const formFields = [{ prop: 'channelName', label: '渠道名称', required: true }, { prop: 'channelType', label: '渠道类型' }, { prop: 'ownerTeam', label: '归属团队' }, { prop: 'status', label: '状态', type: 'select', options: statusOptions }, { prop: 'remark', label: '备注', type: 'textarea', rows: 3 }]
const rowActions = []
