  ...
  <div class="login-page">
    ...
    <el-card class="login-card" shadow="always">
      <div class="logo-section">
        ...
        <h2 class="title">API接口文档与测试管理</h2>
        <p class="subtitle">接口定义、Mock规则、测试用例、执行记录和文档快照协同管理</p>
      </div>
      ...
      <div class="accounts">
        ...
        <div class="account-tags">
          ...
          <el-tag type="success" effect="plain" round>product / 123456</el-tag>
          <el-tag type="warning" effect="plain" round>tester / 123456</el-tag>
          <el-tag type="danger" effect="plain" round>developer / 123456</el-tag>
        </div>
      </div>

      <div class="api-tips">
        <span>🔗 API驱动 · 质量保障 🔗</span>
      </div>
    </el-card>
  </div>
</template>
...
.login-page {
  ...
  align-items: center;
  background: linear-gradient(135deg, #6C63FF 0%, #7C4DFF 30%, #B388FF 60%, #E1BEE7 100%);
  position: relative;
  overflow: hidden;
}
...
.login-card::before {
  ...
  right: 0;
  height: 6px;
  background: linear-gradient(90deg, #6C63FF, #7C4DFF, #B388FF, #6C63FF);
  background-size: 300% 100%;
  animation: apiGradient 4s linear infinite;
}
...
.title {
  font-size: 26px;
  font-weight: 800;
  background: linear-gradient(135deg, #4A148C, #6C63FF, #7C4DFF);
...
.submit-btn {
  ...
  background: linear-gradient(135deg, #4A148C 0%, #6C63FF 50%, #7C4DFF 100%) !important;
