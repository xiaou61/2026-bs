  ...
  <div class="page-container">
    <el-card>
      <div class="search-bar">
        <el-input v-model="query.name" placeholder="产品名称" style="width:200px" clearable />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id?'编辑产品':'新增产品'" width="500px">
      <el-form :model="form" ref="formRef" label-width="80px">
        <el-form-item label="产品编号" prop="code" :rules="[{required:true,message:'请输入'}]"><el-input v-model="form.code" /></el-form-item>
        <el-form-item label="产品名称" prop="name" :rules="[{required:true,message:'请输入'}]"><el-input v-model="form.name" /></el-form-item>
...
const form = reactive({ id: null, code: '', name: '', spec: '', unit: '' })

const loadData = async () => { loading.value = true; try { const res = await getProductPage(query); tableData.value = res.data.records; total.value = res.data.total } finally { loading.value = false } }
const handleAdd = () => { Object.assign(form, { id: null, code: '', name: '', spec: '', unit: '' }); dialogVisible.value = true }
const handleEdit = (row) => { Object.assign(form, row); dialogVisible.value = true }
const handleSubmit = async () => { await formRef.value.validate(); if (form.id) { await updateProduct(form) } else { await addProduct(form) }; ElMessage.success('操作成功'); dialogVisible.value = false; loadData() }
