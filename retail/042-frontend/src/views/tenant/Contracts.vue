  ...
  <div class="contracts-page">
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="操作" width="100">
        <template #default="{ row }">
          <el-button v-if="row.status === 0" size="small" type="primary" @click="handleSign(row)">签署</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const handleSign = async (row) => {
  await ElMessageBox.confirm(`确认签署合同？签署后将支付首期租金 ¥${row.monthlyRent}`, '签署合同')
