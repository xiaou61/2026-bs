<template><DataPage title="账号权限" description="门店店长、理货员、库存主管、促销专员、供应商协同员账号与状态维护" :api="api" :columns="columns" :form-fields="formFields" :manage-roles="['ADMIN']" :delete-roles="['ADMIN']" :status-options="statusOptions" :default-status="1" /></template>
<script setup>
