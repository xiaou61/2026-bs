  ...
  <div class="page-container">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="role" label="角色">
          <template #default="{ row }">
            <el-tag :type="row.role === 'ADMIN' ? 'danger' : row.role === 'DELIVERY' ? 'warning' : 'info'">
              {{ { ADMIN: '管理员', DELIVERY: '配送员', USER: '用户' }[row.role] }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑用户' : '新增用户'" width="500px">
      <el-form :model="form" ref="formRef" label-width="80px" :rules="rules">
        ...
        <el-form-item label="密码" prop="password"><el-input v-model="form.password" /></el-form-item>
...
const form = reactive({ id: null, username: '', password: '', nickname: '', phone: '', role: 'USER', status: 1 })
const rules = {
...
const handleAdd = () => {
  Object.assign(form, { id: null, username: '', password: '', nickname: '', phone: '', role: 'USER', status: 1 })
