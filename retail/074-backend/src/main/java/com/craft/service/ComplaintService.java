...
public class ComplaintService {
    ...
    public void add(Complaint complaint, Long userId) {
        ...
        CraftOrder order = orderMapper.selectById(complaint.getOrderId());
        if (order == null) {
            throw new BusinessException("订单不存在");
        }
        if (!order.getUserId().equals(userId)) {
            throw new BusinessException(403, "只能申诉自己的订单");
        }
    ...
    private Page<ComplaintVO> convertComplaintPage(Page<Complaint> page) {
        ...
        Map<Long, CraftOrder> orderMap = new HashMap<>();
        Set<Long> itemIds = new HashSet<>();
        if (!orderIds.isEmpty()) {
            List<CraftOrder> orders = orderMapper.selectBatchIds(orderIds);
        ...
        for (Complaint complaint : page.getRecords()) {
            ...
            CraftOrder order = orderMap.get(complaint.getOrderId());
            if (order != null) {
    ...
    private void appendOrderNoFilter(QueryWrapper<Complaint> wrapper, String orderNo) {
        ...
        List<Long> orderIds = orders.stream().map(CraftOrder::getId).collect(Collectors.toList());
