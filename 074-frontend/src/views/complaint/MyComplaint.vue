<template>
  <el-card>
    ...
    <el-table :data="tableData" v-loading="loading">
      ...
      <el-table-column prop="itemTitle" label="商品" min-width="160" />
  ...
  <el-dialog v-model="dialogVisible" title="新增申诉" width="520px">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="关联订单" prop="orderId">
        <el-select v-model="form.orderId" style="width: 100%" filterable placeholder="请选择订单">
          <el-option v-for="order in orderOptions" :key="order.id" :label="`${order.orderNo} / ${order.itemTitle || '商品'}`" :value="order.id" />
        </el-select>
      </el-form-item>
