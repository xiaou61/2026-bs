<template><DataPage title="账号权限" description="社区管理员、分类督导员、居民用户、志愿协管员、积分兑换员账号与状态维护" :api="api" :columns="columns" :form-fields="formFields" :manage-roles="['ADMIN']" :delete-roles="['ADMIN']" :status-options="statusOptions" :default-status="1" /></template>
<script setup>
