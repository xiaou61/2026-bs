...
public interface BudgetAllocationMapper {
    ...
    List<BudgetAllocation> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM budget_allocation WHERE id = #{id}")
    BudgetAllocation selectById(Long id);
    ...
    int insert(BudgetAllocation entity);
    ...
    int update(BudgetAllocation entity);
