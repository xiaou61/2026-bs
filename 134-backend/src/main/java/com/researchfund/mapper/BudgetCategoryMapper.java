...
public interface BudgetCategoryMapper {
    ...
    List<BudgetCategory> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM budget_category WHERE id = #{id}")
    BudgetCategory selectById(Long id);
    ...
    int insert(BudgetCategory entity);
    ...
    int update(BudgetCategory entity);
