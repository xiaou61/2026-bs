...
@Mapper
public interface PerformanceStatisticMapper {
    @Select({
        "<script>",
    ...
    List<PerformanceStatistic> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM performance_statistic WHERE id = #{id}")
    PerformanceStatistic selectById(Long id);
    ...
    int insert(PerformanceStatistic entity);
    ...
    int update(PerformanceStatistic entity);
