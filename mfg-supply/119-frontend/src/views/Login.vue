  ...
  <div class="login">
    ...
    <el-card class="login-card" shadow="always">
      <div class="logo-section">
        ...
        <h2 class="title">设备备件寿命预测与维保决策</h2>
        <p class="subtitle">多角色后台管理、状态流转、统计看板和操作审计闭环</p>
      </div>
      ...
      <div class="accounts">
        ...
        <div class="account-tags">
          ...
          <el-tag type="success" effect="plain" round>equipment / 123456</el-tag>
          <el-tag type="warning" effect="plain" round>maintainer / 123456</el-tag>
          <el-tag type="info" effect="plain" round>analyst / 123456</el-tag>
        </div>
      </div>

      <div class="maintenance-tips">
        <span>⚙️ 预测维护 · 降本增效 ⚙️</span>
      </div>
    </el-card>
  </div>
</template>
...
.login {
  ...
  align-items: center;
  background: linear-gradient(135deg, #37474F 0%, #455A64 30%, #546E7A 60%, #78909C 100%);
  position: relative;
  overflow: hidden;
}
...
.login-card {
  ...
  border-radius: 28px;
  box-shadow: 0 24px 64px rgba(55, 71, 79, 0.3);
  position: relative;
  z-index: 10;
...
.login-card::before {
  ...
  animation: maintenanceGradient 4s linear infinite;
}
...
.login-form :deep(.el-input__wrapper) {
  ...
  box-shadow: 0 2px 12px rgba(55, 71, 79, 0.08) !important;
  padding: 4px 12px;
  transition: all 0.3s ease;
}

.login-form :deep(.el-input__wrapper:hover) {
  box-shadow: 0 4px 16px rgba(55, 71, 79, 0.15) !important;
}

.login-form :deep(.el-input__wrapper.is-focus) {
  box-shadow: 0 4px 20px rgba(55, 71, 79, 0.25) !important;
}
