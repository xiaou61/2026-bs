<template>
  <DataPage title="账号权限" description="管理员、设备管理员、维保员、分析员四角色账号和启停状态管理" :api="api" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="defaults" @row-action="handleAction" />
</template>
...
const formFields = [{"prop": "username", "label": "用户名"}, {"prop": "password", "label": "密码"}, {"prop": "nickname", "label": "姓名"}, {"prop": "role", "label": "角色", "type": "select", "options": [{"label": "ADMIN", "value": "ADMIN"}, {"label": "EQUIPMENT", "value": "EQUIPMENT"}, {"label": "MAINTAINER", "value": "MAINTAINER"}, {"label": "ANALYST", "value": "ANALYST"}]}, {"prop": "department", "label": "部门"}, {"prop": "phone", "label": "手机"}, {"prop": "email", "label": "邮箱"}, {"prop": "status", "label": "状态", "type": "select", "options": [{"label": "1", "value": 1}, {"label": "0", "value": 0}]}]
const rowActions = [{"command": "enable", "label": "启用", "type": "success"}, {"command": "disable", "label": "停用", "type": "danger"}]
const defaults = {"role": "EQUIPMENT", "status": 1}
