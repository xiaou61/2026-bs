<template><DataPage title="账号权限" description="调度主管、门岗核验员、场内调度员、承运商代表账号与状态维护" :api="api" :columns="columns" :form-fields="formFields" :manage-roles="['ADMIN']" :delete-roles="['ADMIN']" :status-options="statusOptions" :default-status="1" /></template>
<script setup>
