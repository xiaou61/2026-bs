...
@Mapper
public interface ViolationAppealMapper {
    @Select({
        "<script>",
    ...
    List<ViolationAppeal> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM performance_statistic WHERE id = #{id}")
    ViolationAppeal selectById(Long id);
    ...
    int insert(ViolationAppeal entity);
    ...
    int update(ViolationAppeal entity);
