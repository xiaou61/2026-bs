...
@Mapper
public interface SuspiciousBehaviorMapper {
    @Select({
        "<script>",
    ...
    List<SuspiciousBehavior> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM invoice_record WHERE id = #{id}")
    SuspiciousBehavior selectById(Long id);
    ...
    int insert(SuspiciousBehavior entity);
    ...
    int update(SuspiciousBehavior entity);
