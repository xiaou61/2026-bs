...
@Mapper
public interface ReviewDecisionMapper {
    @Select({
        "<script>",
    ...
    List<ReviewDecision> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM research_achievement WHERE id = #{id}")
    ReviewDecision selectById(Long id);
    ...
    int insert(ReviewDecision entity);
    ...
    int update(ReviewDecision entity);
