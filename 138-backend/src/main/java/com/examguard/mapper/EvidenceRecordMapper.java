...
public interface EvidenceRecordMapper {
    ...
    List<EvidenceRecord> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM approval_task WHERE id = #{id}")
    EvidenceRecord selectById(Long id);
    ...
    int insert(EvidenceRecord entity);
    ...
    int update(EvidenceRecord entity);
