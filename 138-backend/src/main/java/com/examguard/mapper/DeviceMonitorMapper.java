...
public interface DeviceMonitorMapper {
    ...
    List<DeviceMonitor> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM patent_record WHERE id = #{id}")
    DeviceMonitor selectById(Long id);
    ...
    int insert(DeviceMonitor entity);
    ...
    int update(DeviceMonitor entity);
