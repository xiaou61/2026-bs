...
public interface AlertNotificationMapper {
    ...
    List<AlertNotification> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM risk_warning WHERE id = #{id}")
    AlertNotification selectById(Long id);
    ...
    int insert(AlertNotification entity);
    ...
    int update(AlertNotification entity);
