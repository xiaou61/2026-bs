...
@Mapper
public interface TopicApplicationMapper {
    @Select({
        "<script>",
    ...
    List<TopicApplication> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM expense_claim WHERE id = #{id}")
    TopicApplication selectById(Long id);
    ...
    int insert(TopicApplication entity);
    ...
    int update(TopicApplication entity);
