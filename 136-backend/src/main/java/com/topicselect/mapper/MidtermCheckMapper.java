...
@Mapper
public interface MidtermCheckMapper {
    @Select({
        "<script>",
    ...
    List<MidtermCheck> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM patent_record WHERE id = #{id}")
    MidtermCheck selectById(Long id);
    ...
    int insert(MidtermCheck entity);
    ...
    int update(MidtermCheck entity);
