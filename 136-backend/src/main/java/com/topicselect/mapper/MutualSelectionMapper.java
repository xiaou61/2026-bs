...
@Mapper
public interface MutualSelectionMapper {
    @Select({
        "<script>",
    ...
    List<MutualSelection> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM approval_task WHERE id = #{id}")
    MutualSelection selectById(Long id);
    ...
    int insert(MutualSelection entity);
    ...
    int update(MutualSelection entity);
