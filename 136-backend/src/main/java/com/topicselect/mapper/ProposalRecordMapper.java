...
@Mapper
public interface ProposalRecordMapper {
    @Select({
        "<script>",
    ...
    List<ProposalRecord> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM research_achievement WHERE id = #{id}")
    ProposalRecord selectById(Long id);
    ...
    int insert(ProposalRecord entity);
    ...
    int update(ProposalRecord entity);
