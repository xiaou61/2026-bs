  ...
  <div>
    <el-card>
      <div style="display:flex;gap:10px;margin-bottom:15px">
        <el-input v-model="query.name" placeholder="搜索分类" style="width:200px" clearable />
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="status" label="状态"><template #default="{ row }"><el-tag :type="row.status === 1 ? 'success' : 'info'">{{ row.status === 1 ? '启用' : '禁用' }}</el-tag></template></el-table-column>
...
const loadData = async () => {
  loading.value = true
  try { const res = await getCategoryPage(query); tableData.value = res.data.list; total.value = res.data.total } finally { loading.value = false }
}
