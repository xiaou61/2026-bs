<template>
  <div>
    <el-card>
      <div class="toolbar">
        <el-input v-model="query.orderNo" placeholder="消费单号" clearable style="width: 220px" />
      ...
      <el-table :data="tableData" border v-loading="loading">
        ...
        <el-table-column prop="orderNo" label="消费单号" width="210" />
        <el-table-column prop="userId" label="会员ID" width="90" />
        <el-table-column prop="movieTitle" label="影片" min-width="140" />
        <el-table-column prop="seatInfo" label="座位信息" min-width="200" />
...
const handleCancel = async id => {
  ...
  ElMessage.success('消费单已取消')
  loadData()
}

const handleRefund = async id => {
  await refundOrder(id)
  ElMessage.success('消费单已退款')
  loadData()
}
