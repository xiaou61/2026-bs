...
public class UserService {
    ...
    public Long countFans() {
        return userMapper.selectCount(new LambdaQueryWrapper<User>().eq(User::getRole, "FAN"));
    }
    ...
    private void update(User user) {
        ...
        if (StringUtils.hasText(user.getRole())) {
            ...
            if (!"ADMIN".equals(role) && !"MANAGER".equals(role) && !"FAN".equals(role)) {
    ...
    private void prepareUserData(User user, boolean createUserNo) {
        String role = StringUtils.hasText(user.getRole()) ? user.getRole().trim().toUpperCase() : "FAN";
        if (!"ADMIN".equals(role) && !"MANAGER".equals(role) && !"FAN".equals(role)) {
            throw new BusinessException("角色不合法");
        }
        user.setRole(role);
