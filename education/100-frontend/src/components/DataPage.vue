  ...
  <div class="page-shell">
    <div class="page-title">
      ...
      <div class="title-actions">
        <el-button v-for="action in toolActions" :key="action.name" :type="action.type || 'primary'" :icon="action.icon" @click="$emit('tool-action', action.name)">{{ action.label }}</el-button>
    ...
    <div class="table-panel">
      ...
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%; margin-top: 14px">
        ...
        <el-table-column v-if="rowActions.length || !readonly" label="操作" width="240" fixed="right">
          <template #default="{ row }">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑' : '新增'" :width="dialogWidth">
      <el-form ref="formRef" :model="form" label-width="112px">
        <el-form-item v-for="field in formFields" :key="field.prop" :label="field.label" :prop="field.prop" :rules="field.required ? [{ required: true, message: `请输入${field.label}`, trigger: 'blur' }] : []">
          ...
          <el-input-number v-else-if="field.type === 'number'" v-model="form[field.prop]" :min="field.min" :max="field.max" :precision="field.precision" style="width: 100%" />
          <el-select v-else-if="field.type === 'select'" v-model="form[field.prop]" :placeholder="field.placeholder || field.label" style="width: 100%">
            <el-option v-for="option in field.options || []" :key="option.value" :label="option.label" :value="option.value" />
          </el-select>
          <el-switch v-else-if="field.type === 'switch'" v-model="form[field.prop]" :active-value="field.activeValue ?? 1" :inactive-value="field.inactiveValue ?? 0" />
          <el-date-picker v-else-if="field.type === 'date'" v-model="form[field.prop]" type="date" value-format="YYYY-MM-DD" :placeholder="field.placeholder || field.label" style="width: 100%" />
          <el-color-picker v-else-if="field.type === 'color'" v-model="form[field.prop]" />
...
defineEmits(['row-action', 'tool-action'])
