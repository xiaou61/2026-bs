  ...
  <div class="dashboard">
    ...
    <div class="panel-grid">
      <el-card class="panel-card" shadow="never">
        <template #header>
          <div class="panel-title">近期课程安排</div>
        </template>
...
import { getDashboard, getPublicNotices, getResourceList, getScheduleList, getStudentSchedule, getTeacherSchedule } from '../api'
...
const hero = computed(() => {
  if (userStore.role === 'student') {
    return {
      ...
      title: '今天先看课表，再拿资源，最后把评教补完。',
      desc: '学生端聚合选课、资源、成绩与评教入口，保持学习任务一屏完成。'
    }
  }
  if (userStore.role === 'teacher') {
    return {
      eyebrow: 'Teaching Workflow',
      title: '从授课排期到考勤成绩，教学过程可以顺滑闭环。',
      desc: '教师端重点保留课程安排、资源发放、考勤与成绩录入的高频操作。'
    }
  }
  return {
    eyebrow: 'Admin Operations',
    title: '院系、课程、排课与公告在同一套节奏里协同运转。',
    desc: '管理员看板关注教务总览与组织结构维护，让课程流程从配置开始就保持清晰。'
  }
})
...
const cards = computed(() => {
  if (userStore.role === 'student') {
    return [
      { label: '已选课程', value: stats.value.scheduleCount || 0 },
  ...
  return [
    ...
    { label: '学生人数', value: stats.value.studentCount || 0 },
    { label: '课程总数', value: stats.value.courseCount || 0 },
    { label: '排课总数', value: stats.value.scheduleCount || 0 },
    { label: '选课总数', value: stats.value.selectionCount || 0 }
  ]
})

const loadStudentData = async () => {
  const [scheduleRes, resourceRes, noticeRes] = await Promise.all([
    getStudentSchedule(),
  ...
  stats.value = { scheduleCount: scheduleList.value.length }
}
