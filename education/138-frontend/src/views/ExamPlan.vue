...
import { getExamPlanPage, addExamPlan, updateExamPlan, deleteExamPlan, activateExamPlan, finishExamPlan } from '../api'
const api = { page: getExamPlanPage, add: addExamPlan, update: updateExamPlan, delete: deleteExamPlan }
...
const handleAction = async ({ command, row, refresh }) => {
  if (command === 'activate') await activateExamPlan(row.id)
  if (command === 'finish') await finishExamPlan(row.id)
