...
public interface PaperRecordMapper {
    ...
    List<PaperRecord> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM paper_record WHERE id = #{id}")
    PaperRecord selectById(Long id);
