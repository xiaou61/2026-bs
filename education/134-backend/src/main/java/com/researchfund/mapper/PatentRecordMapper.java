...
public interface PatentRecordMapper {
    ...
    List<PatentRecord> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM patent_record WHERE id = #{id}")
    PatentRecord selectById(Long id);
    ...
    int insert(PatentRecord entity);
    ...
    int update(PatentRecord entity);
