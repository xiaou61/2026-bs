...
public interface ResearchProjectMapper {
    ...
    List<ResearchProject> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM research_project WHERE id = #{id}")
    ResearchProject selectById(Long id);
    ...
    int insert(ResearchProject entity);
    ...
    int update(ResearchProject entity);
