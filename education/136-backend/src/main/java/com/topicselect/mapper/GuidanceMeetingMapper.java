...
@Mapper
public interface GuidanceMeetingMapper {
    @Select({
        "<script>",
    ...
    List<GuidanceMeeting> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM performance_statistic WHERE id = #{id}")
    GuidanceMeeting selectById(Long id);
    ...
    int insert(GuidanceMeeting entity);
    ...
    int update(GuidanceMeeting entity);
