...
@Mapper
public interface MilestoneNoticeMapper {
    @Select({
        "<script>",
    ...
    List<MilestoneNotice> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM risk_warning WHERE id = #{id}")
    MilestoneNotice selectById(Long id);
    ...
    int insert(MilestoneNotice entity);
    ...
    int update(MilestoneNotice entity);
