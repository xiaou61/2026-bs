...
@Mapper
public interface OpeningDefenseMapper {
    @Select({
        "<script>",
    ...
    List<OpeningDefense> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM paper_record WHERE id = #{id}")
    OpeningDefense selectById(Long id);
    ...
    int insert(OpeningDefense entity);
    ...
    int update(OpeningDefense entity);
