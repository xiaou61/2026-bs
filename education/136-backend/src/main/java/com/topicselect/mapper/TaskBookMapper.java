...
public interface TaskBookMapper {
    ...
    List<TaskBook> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM payment_record WHERE id = #{id}")
    TaskBook selectById(Long id);
