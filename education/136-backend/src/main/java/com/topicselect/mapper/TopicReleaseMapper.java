...
@Mapper
public interface TopicReleaseMapper {
    @Select({
        "<script>",
    ...
    List<TopicRelease> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM research_project WHERE id = #{id}")
    TopicRelease selectById(Long id);
    ...
    int insert(TopicRelease entity);
    ...
    int update(TopicRelease entity);
