...
@Mapper
public interface InvigilatorProfileMapper {
    @Select({
        "<script>",
    ...
    List<InvigilatorProfile> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM budget_category WHERE id = #{id}")
    InvigilatorProfile selectById(Long id);
    ...
    int insert(InvigilatorProfile entity);
    ...
    int update(InvigilatorProfile entity);
