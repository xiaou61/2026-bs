...
public interface CandidateProfileMapper {
    ...
    List<CandidateProfile> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM budget_allocation WHERE id = #{id}")
    CandidateProfile selectById(Long id);
    ...
    int insert(CandidateProfile entity);
    ...
    int update(CandidateProfile entity);
