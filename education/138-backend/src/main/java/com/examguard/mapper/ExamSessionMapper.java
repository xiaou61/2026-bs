...
public interface ExamSessionMapper {
    ...
    List<ExamSession> selectPage(@Param("keyword") String keyword, @Param("status") String status);

    @Select("SELECT * FROM expense_claim WHERE id = #{id}")
    ExamSession selectById(Long id);
