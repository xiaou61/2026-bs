...
public class NewsController {
    ...
    @PutMapping
    public Result<?> update(@RequestBody News news, HttpServletRequest request) {
        AuthUtils.requireAdmin(request);
    ...
    public Result<?> deleteComment(@PathVariable Long id, HttpServletRequest request) {
        newsService.deleteComment(id, AuthUtils.getUserId(request), AuthUtils.isAdmin(request));
        return Result.success();
    }
}
