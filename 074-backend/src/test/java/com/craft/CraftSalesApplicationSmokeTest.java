...
class CraftSalesApplicationSmokeTest {
    ...
    void smokeTestLoginAuthorizationOrderReviewComplaintAndLogout() {
        ...
        ResponseEntity<Map> orderResponse = restTemplate.exchange(
                "/api/order",
                HttpMethod.POST,
                authEntity(buyer.token(), Map.of("itemId", 3, "quantity", 1, "remark", "gift package")),
                Map.class
        );
        ...
        assertOk(restTemplate.exchange(
                "/api/review",
                HttpMethod.POST,
                authEntity(buyer.token(), Map.of("orderId", orderId.longValue(), "rating", 5, "content", "excellent craft")),
                Map.class
        ));
