<template>
  <DataPage ref="pageRef" title="委托人档案" description="维护委托人实名信息、联系方式和偏好案件类型。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ status: 1 }" @row-action="handleAction" />
</template>
...
import { addClientProfile, deleteClientProfile, getClientProfilePage, updateClientProfile } from '../api'
...
const api = { page: getClientProfilePage, add: addClientProfile, update: updateClientProfile, delete: deleteClientProfile }
const filters = [{ type: 'input', prop: 'keyword', label: '关键词' }, { type: 'select', prop: 'status', label: '状态', options: statusOptions }]
const columns = [{ prop: 'realName', label: '姓名' }, { prop: 'phone', label: '电话' }, { prop: 'address', label: '地址', minWidth: 180 }, { prop: 'casePreference', label: '偏好类型' }, { prop: 'status', label: '状态', map: statusMap }]
const formFields = [{ prop: 'userId', label: '用户ID', type: 'number', min: 1 }, { prop: 'realName', label: '姓名', required: true }, { prop: 'idNo', label: '证件号' }, { prop: 'phone', label: '电话' }, { prop: 'address', label: '地址' }, { prop: 'casePreference', label: '偏好类型' }, { prop: 'status', label: '状态', type: 'select', options: statusOptions }]
const rowActions = []
