...
import com.heritageworkshop.mapper.BookingReviewMapper;
...
public class BookingReviewService {
    ...
    public BookingReview getById(Long id) {
        ...
        if (entity == null) throw new BusinessException("预约审核不存在");
        return entity;
    }

    public void save(BookingReview entity) {
        if (!StringUtils.hasText(entity.getRecordName())) throw new BusinessException("审核名称不能为空");
