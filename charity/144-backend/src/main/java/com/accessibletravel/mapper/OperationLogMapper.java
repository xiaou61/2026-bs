...
import com.accessibletravel.entity.OperationLog;
...
public interface OperationLogMapper {
    ...
    @Insert("INSERT INTO operation_log (operator_name, module_name, action_type, target_name, detail_info, status, created_time, updated_time) VALUES (#{operatorName}, #{moduleName}, #{actionType}, #{targetName}, #{detailInfo}, #{status}, NOW(), NOW())")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(OperationLog entity);

    @Update("UPDATE operation_log SET operator_name = #{operatorName}, module_name = #{moduleName}, action_type = #{actionType}, target_name = #{targetName}, detail_info = #{detailInfo}, status = #{status}, updated_time = NOW() WHERE id = #{id}")
