...
public class UserService {
    ...
    public Map<String, Object> login(String username, String password) {
        ...
        User user = userMapper.selectOne(new LambdaQueryWrapper<User>().eq(User::getUsername, username.trim()).last("limit 1"));
        if (user == null) {
        ...
        redisTemplate.opsForValue().set("blog:token:" + user.getId(), token, 24, TimeUnit.HOURS);
    ...
    public void updatePassword(Long userId, String oldPassword, String newPassword) {
        ...
        redisTemplate.delete("blog:token:" + userId);
    }
    ...
    private void update(User user) {
        ...
        User exist = userMapper.selectOne(new LambdaQueryWrapper<User>().eq(User::getUsername, user.getUsername().trim()).last("limit 1"));
        ...
        userMapper.updateById(db);
        if (invalidate) {
            redisTemplate.delete("blog:token:" + db.getId());
        }
    }
    ...
    public void updateStatus(Long id, Integer status) {
        ...
        userMapper.updateById(db);
        if (status == 0) {
            redisTemplate.delete("blog:token:" + id);
        }
    }
