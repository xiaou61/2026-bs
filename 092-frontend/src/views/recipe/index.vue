<template>
  <div class="page">
    <div class="toolbar">
      <el-form :inline="true" :model="filters">
        <el-form-item label="食谱标题">
          <el-input v-model="filters.keyword" placeholder="请输入食谱标题" clearable />
        </el-form-item>
        <el-form-item label="所属排课">
      ...
      <div class="toolbar-actions">
        ...
        <el-button v-if="canEdit" type="success" @click="openDialog()">新增食谱</el-button>
      </div>
    </div>

    <el-card shadow="never">
      <el-table :data="list" stripe>
        <el-table-column prop="title" label="食谱标题" min-width="180" />
        ...
        <el-table-column label="食谱链接" min-width="220">
          <template #default="{ row }">
            <a :href="row.resourceUrl" target="_blank" class="link">{{ row.resourceUrl }}</a>
          </template>
        </el-table-column>
        <el-table-column prop="contentDesc" label="食谱说明" min-width="220" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑食谱' : '新增食谱'" width="640px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="92px">
        <el-form-item label="所属排课" prop="scheduleId">
          <el-select v-model="form.scheduleId" placeholder="请选择排课" style="width: 100%" @change="handleScheduleChange">
            <el-option v-for="item in scheduleOptions" :key="item.id" :label="`${item.courseName} / ${item.weekDay}`" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="餐次">
          <el-select v-model="form.resourceType" placeholder="请选择餐次" style="width: 100%">
            <el-option v-for="item in resourceTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="食谱链接">
          <el-input v-model="form.resourceUrl" placeholder="请输入食谱链接" />
        </el-form-item>
        <el-form-item label="食谱说明">
          <el-input v-model="form.contentDesc" type="textarea" :rows="4" placeholder="请输入食谱说明" />
        </el-form-item>
...
import { addResource, deleteResource, getResourceList, getScheduleList, getStudentSchedule, getTeacherSchedule, updateResource } from '../../api'
...
const rules = {
  scheduleId: [{ required: true, message: '请选择所属排课', trigger: 'change' }],
