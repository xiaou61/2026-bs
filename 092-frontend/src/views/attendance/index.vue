  ...
  <div class="page">
    <div class="toolbar">
      <el-form :inline="true" :model="filters">
        ...
        <el-form-item v-if="!isStudent" label="幼儿">
          <el-select v-model="filters.studentId" placeholder="全部幼儿" clearable style="width: 180px">
    ...
    <el-card shadow="never">
      <el-table :data="list" stripe>
        <el-table-column prop="courseName" label="活动名称" min-width="160" />
        <el-table-column prop="studentName" label="幼儿姓名" min-width="120" />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑考勤' : '新增考勤'" width="620px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="92px">
        ...
        <el-form-item label="幼儿" prop="studentId">
          <el-select v-model="form.studentId" placeholder="请选择幼儿" style="width: 100%">
            <el-option v-for="item in selectionOptions" :key="item.studentId" :label="item.studentName" :value="item.studentId" />
          </el-select>
        </el-form-item>
...
import { addAttendance, getAttendanceList, getChildList, getMyChildren, getScheduleList, getTeacherSchedule, updateAttendance } from '../../api'
import { attendanceStatusOptions, getOptionLabel, resolvePage } from '../../utils'
import { useUserStore } from '../../store/user'

const userStore = useUserStore()
const isStudent = computed(() => userStore.role === 'parent')
...
const loadSelectionOptions = async () => {
  ...
  selectionOptions.value = records.map((item) => ({
    studentId: item.parentId,
