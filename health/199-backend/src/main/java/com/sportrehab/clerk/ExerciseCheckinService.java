...
public class ExerciseCheckinService {
    private final ExerciseCheckinMapper mapper;

    public PageInfo<ExerciseCheckin> page(Integer pageNum, Integer pageSize, String keyword, String status) {
        PageHelper.startPage(pageNum == null ? 1 : pageNum, pageSize == null ? 10 : pageSize);
        return new PageInfo<>(mapper.selectPage(keyword, status));
    }

    public ExerciseCheckin getById(Long id) {
        ExerciseCheckin entity = mapper.selectById(id);
        if (entity == null) {
            throw new BusinessException("运动打卡记录不存在");
        }
        return entity;
    }

    public void save(ExerciseCheckin entity) {
