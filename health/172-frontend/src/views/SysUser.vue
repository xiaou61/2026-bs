<template><DataPage title="账号权限" description="前台导诊、口腔医生、诊疗护士、收费人员、患者用户账号与状态维护" :api="api" :columns="columns" :form-fields="formFields" :manage-roles="['ADMIN']" :delete-roles="['ADMIN']" :status-options="statusOptions" :default-status="1" /></template>
<script setup>
