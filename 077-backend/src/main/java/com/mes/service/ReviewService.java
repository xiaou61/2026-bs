...
import com.mes.mapper.ReviewMapper;
...
public class ReviewService {
    ...
    public List<ReviewVO> listByItem(Long serviceId) {
        if (serviceId == null) {
            throw new BusinessException("生产任务ID不能为空");
        }
    ...
    public void add(Review review, Long userId) {
        if (review == null || review.getOrderId() == null) {
            throw new BusinessException("执行工单ID不能为空");
        }
        ...
        if (!order.getUserId().equals(userId)) {
            throw new BusinessException(403, "无权限评价该执行工单");
        }
        if (order.getStatus() != 3) {
            throw new BusinessException("执行工单未完成，无法评价");
        }
        Long count = reviewMapper.selectCount(new QueryWrapper<Review>()
                .eq("order_id", review.getOrderId())
                .eq("user_id", userId));
        if (count > 0) {
            throw new BusinessException("该执行工单已评价");
        }
