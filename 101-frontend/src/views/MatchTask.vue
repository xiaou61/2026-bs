<template>
  <DataPage ref="pageRef" title="匹配任务" description="为候选人和岗位创建匹配任务并生成推荐结果。" :api="api" :filters="filters" :columns="columns" :form-fields="formFields" :row-actions="rowActions" :defaults="{ priority: '普通' }" @row-action="handleAction" />
</template>
...
const formFields = [{ prop: 'taskName', label: '任务名称', required: true }, { prop: 'candidateId', label: '候选人ID', type: 'number', min: 1, required: true }, { prop: 'jobId', label: '岗位ID', type: 'number', min: 1, required: true }, { prop: 'priority', label: '优先级', type: 'select', options: priorityOptions }]
