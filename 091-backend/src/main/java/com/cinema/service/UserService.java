...
public class UserService {
    ...
    public void addRechargeAmount(Long userId, BigDecimal amount) {
        ...
        BigDecimal totalRecharge = user.getTotalRecharge() == null ? BigDecimal.ZERO : user.getTotalRecharge();
        user.setTotalRecharge(totalRecharge.add(amount));
    ...
    public void addConsumeAmount(Long userId, BigDecimal amount) {
        ...
        user.setPoints(points + amount.intValue());
        refreshMemberLevel(user);
        userMapper.updateById(user);
    }
    ...
    private void add(User user) {
        ...
        user.setEmail(StringUtils.hasText(user.getEmail()) ? user.getEmail().trim() : null);
        user.setAvatar(StringUtils.hasText(user.getAvatar()) ? user.getAvatar().trim() : null);
        user.setRole(StringUtils.hasText(user.getRole()) ? user.getRole().trim().toUpperCase() : "MEMBER");
        user.setStatus(user.getStatus() == null ? 1 : user.getStatus());
        prepareMemberData(user, false);
        userMapper.insert(user);
    }
    ...
    private void update(User user) {
        ...
        if (StringUtils.hasText(user.getRole())) {
            ...
            if (!"ADMIN".equals(role) && !"STAFF".equals(role) && !"MEMBER".equals(role)) {
        ...
        if (StringUtils.hasText(user.getMemberLevel())) {
            db.setMemberLevel(user.getMemberLevel().trim().toUpperCase());
        } else {
            refreshMemberLevel(db);
        }
        if (user.getPoints() != null) {
            db.setPoints(user.getPoints());
        }
        if (user.getTotalRecharge() != null) {
            db.setTotalRecharge(user.getTotalRecharge());
        }
    ...
    private void prepareMemberData(User user, boolean createMemberNo) {
        if (!StringUtils.hasText(user.getRole())) {
            user.setRole("MEMBER");
        }
        String role = user.getRole().trim().toUpperCase();
        ...
        user.setRole(role);
        if (createMemberNo || !StringUtils.hasText(user.getMemberNo())) {
            user.setMemberNo(buildMemberNo());
        }
        user.setBalance(user.getBalance() == null ? BigDecimal.ZERO : user.getBalance());
        user.setPoints(user.getPoints() == null ? 0 : user.getPoints());
        user.setTotalRecharge(user.getTotalRecharge() == null ? BigDecimal.ZERO : user.getTotalRecharge());
    ...
    private void refreshMemberLevel(User user) {
        ...
        if (!"MEMBER".equals(user.getRole())) {
            user.setMemberLevel(user.getRole());
            return;
        }
        if (StringUtils.hasText(user.getMemberLevel()) && !"SILVER".equalsIgnoreCase(user.getMemberLevel())
                && !"GOLD".equalsIgnoreCase(user.getMemberLevel())
                && !"DIAMOND".equalsIgnoreCase(user.getMemberLevel())) {
            user.setMemberLevel("SILVER");
            return;
        }
        BigDecimal recharge = user.getTotalRecharge() == null ? BigDecimal.ZERO : user.getTotalRecharge();
        BigDecimal consume = user.getTotalConsume() == null ? BigDecimal.ZERO : user.getTotalConsume();
        BigDecimal score = recharge.add(consume);
        if (score.compareTo(new BigDecimal("5000")) >= 0) {
            user.setMemberLevel("DIAMOND");
        } else if (score.compareTo(new BigDecimal("2000")) >= 0) {
