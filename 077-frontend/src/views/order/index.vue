<template>
  <el-card>
    <div class="bar">
      <el-input v-model="query.orderNo" placeholder="执行工单号" style="width: 220px" clearable />
    ...
    <el-table :data="tableData" v-loading="loading">
      <el-table-column prop="orderNo" label="执行工单号" min-width="220" />
      <el-table-column prop="buyerName" label="发起人" width="120" />
      <el-table-column prop="sellerName" label="生产方" width="120" />
      <el-table-column prop="serviceTitle" label="生产任务" min-width="180" />
      <el-table-column prop="quantity" label="投产数量" width="90" />
      ...
      <el-table-column label="操作" width="180">
        <template #default="{ row }">
          ...
          <el-button link @click="deliver(row)" :disabled="row.status !== 1">确认工单</el-button>
  ...
  <el-dialog v-model="detailVisible" title="工单详情" width="760px">
    <el-descriptions :column="2" border>
      <el-descriptions-item label="执行工单号">{{ detail.orderNo }}</el-descriptions-item>
      <el-descriptions-item label="工单状态">{{ statusText(detail.status) }}</el-descriptions-item>
      <el-descriptions-item label="发起人">{{ detail.buyerName }}</el-descriptions-item>
      <el-descriptions-item label="生产方">{{ detail.sellerName }}</el-descriptions-item>
      <el-descriptions-item label="生产任务">{{ detail.serviceTitle }}</el-descriptions-item>
      <el-descriptions-item label="投产数量">{{ detail.quantity }}</el-descriptions-item>
      <el-descriptions-item label="计划日期">{{ detail.appointmentDate || '-' }}</el-descriptions-item>
      <el-descriptions-item label="计划时间">{{ detail.appointmentTime || '-' }}</el-descriptions-item>
      <el-descriptions-item label="批次号">{{ detail.plateNo || '-' }}</el-descriptions-item>
      <el-descriptions-item label="产品型号">{{ detail.vehicleModel || '-' }}</el-descriptions-item>
      <el-descriptions-item label="工艺说明" :span="2">{{ detail.faultDesc || '-' }}</el-descriptions-item>
