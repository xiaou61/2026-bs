...
import com.aigccopyright.utils.JwtUtils;
...
public class AuthService {
    ...
    public Map<String, Object> login(String username, String password) {
        ...
        operationLogService.record(user.getId(), "认证", "登录", "用户登录系统");
        return result;
    }
    ...
    public void assertAuditor(String role) {
        if (!"ADMIN".equals(role) && !"AUDITOR".equals(role)) {
            throw new BusinessException(403, "无权限操作");
        }
    }

    public void assertCreator(String role) {
        if (!"ADMIN".equals(role) && !"AUDITOR".equals(role) && !"CREATOR".equals(role)) {
            throw new BusinessException(403, "无权限操作");
        }
    }
}
