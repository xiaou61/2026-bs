package com.meddevice.service;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.meddevice.entity.InspectionTask;
import com.meddevice.mapper.InspectionTaskMapper;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;

@Service
@RequiredArgsConstructor
public class InspectionTaskService {
    private final InspectionTaskMapper inspectionTaskMapper;

    public PageInfo<InspectionTask> page(Integer pageNum, Integer pageSize, String keyword, String status) {
        PageHelper.startPage(pageNum == null ? 1 : pageNum, pageSize == null ? 10 : pageSize);
        return new PageInfo<>(inspectionTaskMapper.selectPage(keyword, status));
    }

    public void save(InspectionTask entity) {
        if (entity.getId() == null) inspectionTaskMapper.insert(entity);
        else inspectionTaskMapper.update(entity);
    }

    public void delete(Long id) {
        inspectionTaskMapper.deleteById(id);
    }

    public void updateStatus(Long id, String status) {
        inspectionTaskMapper.updateStatus(id, status);
    }
}
