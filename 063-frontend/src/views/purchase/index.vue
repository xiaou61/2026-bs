<template>
  <el-card>
    <div class="bar">
      <el-input v-model="query.orderNo" placeholder="采购单号" style="width: 220px" clearable />
      <el-select v-model="query.supplierId" placeholder="供应商" style="width: 180px" clearable>
      ...
      <el-button type="success" @click="openAdd">新增采购</el-button>
    </div>

    <el-table :data="tableData" v-loading="loading">
      <el-table-column prop="orderNo" label="采购单号" min-width="190" />
  ...
  <el-dialog v-model="dialogVisible" :title="form.id ? '编辑采购单' : '新增采购单'" width="620px">
...
import { addPurchase, approvePurchase, deletePurchase, getProductList, getPurchasePage, getSupplierList, updatePurchase } from '../../api'
...
const formRef = ref()
const query = reactive({ pageNum: 1, pageSize: 10, orderNo: '', supplierId: null, status: null })
const form = reactive({})
...
const loadBase = async () => {
  const [sRes, pRes] = await Promise.all([getSupplierList(), getProductList()])
...
const openAdd = async () => {
  await loadBase()
  Object.assign(form, { id: null, supplierId: null, productId: null, quantity: 1, price: 1, remark: '' })
